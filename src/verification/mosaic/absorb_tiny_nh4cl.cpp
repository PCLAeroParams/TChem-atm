...
void absorb_tiny_nh4cl(Ensemble *ensemble) {
  ensemble->process([=](const Input &input, Output &output) {
    ...
    const auto delta_hcl_max_arr   = input.get_array("delta_hcl_max");
    ...
    real_type delta_hcl_max_val   = delta_hcl_max_arr[0];
    ...
    KOKKOS_LAMBDA(const typename policy_type::member_type& member) {
      TChem::Impl::MOSAIC<real_type, device_type>::absorb_tiny_nh4cl(
        mmd,
        aer_solid,
        aer_liquid,
        aer_total,
    ...
    std::vector<Real> aer_solid_out(mmd.naer), aer_liquid_out(mmd.naer),
                      ...
                      delta_nh3_max_out(1), delta_hcl_max_out(1), electrolyte_sum_out(1);
    ...
    verification::convert_1d_view_device_to_1d_vector(delta_hcl_max_view, delta_hcl_max_out);
    ...
    output.set("delta_hcl_max", delta_hcl_max_out);

...
void
AtmosphericChemistryE3SM_TemplateRunModelVariation( /// required template arguments
  ...
  const Kokkos::View<KineticModelNCAR_ConstData<DeviceType >*,DeviceType>& kmcds)
{
  ...
  const ordinal_type per_team_extent = AtmosphericChemistryE3SM::getWorkSpaceSize(kmcd_host());
  ...
  Kokkos::parallel_for(
	...
	const real_type t_beg = tadv_at_i._tbeg;
        ...
        Impl::AtmosphericChemistryE3SM<ValueType,device_type>;
...
template<typename PolicyType,
         ...
         typename TimeAdvance1DViewType>
void
AtmosphericChemistryE3SM_TemplateRun( /// required template arguments
  const std::string& profile_name,
  const ValueType& dummyValueType,
  /// team size setting
  const PolicyType& policy,
  ...
  const KineticModelNCAR_ConstData<DeviceType>& kmcd)
{
  ...
  Kokkos::View<KineticModelNCAR_ConstData<DeviceType>*,DeviceType>
    kmcds(do_not_init_tag("AtmosphericChemistryE3SM::kmcds"), 1);
  Kokkos::deep_copy(kmcds, kmcd);

  AtmosphericChemistryE3SM_TemplateRunModelVariation
    (profile_name,
...
void
AtmosphericChemistryE3SM::runDeviceBatch( /// thread block size
  ...
  const KineticModelNCAR_ConstData<device_type >& kmcd)
{
  ...
  #define TCHEM_RUN_ATMOSPHERIC_CHEMISTRY()                             \
          AtmosphericChemistryE3SM_TemplateRun(                                \
          profile_name,                                                   \
          value_type(),                                                   \
          policy,                                                         \
          tol_newton,                                                     \
...
//
 const std::string profile_name = "TChem::AtmosphericChemistryE3SM::runDeviceBatch::kmcd";
...
//
void
AtmosphericChemistryE3SM::runHostBatch( /// input
...
//
const std::string profile_name = "TChem::AtmosphericChemistryE3SM::runHostBatch::kmcd";
#if defined(TCHEM_ATM_ENABLE_SACADO_JACOBIAN_ATMOSPHERIC_CHEMISTRY)
  ...
  const Kokkos::View<KineticModelNCAR_ConstData<device_type>*,device_type>& kmcds)
{
  ...
  AtmosphericChemistryE3SM_TemplateRunModelVariation(                          \
    ...
    const std::string profile_name = "TChem::AtmosphericChemistryE3SM::runDeviceBatch::kmcd array";
...
void
AtmosphericChemistryE3SM::runHostBatch( /// thread block size
  typename UseThisTeamPolicy<host_exec_space>::type& policy,
  /// input
  const real_type_1d_view_host_type& tol_newton,
  const real_type_2d_view_host_type& tol_time,

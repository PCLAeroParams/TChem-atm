...
=====================================================================================
*/
#ifndef __TCHEM_ATMOSPHERIC_CHEMISTRY_E3SM_IMPLICIT_EULER_HPP__
#define __TCHEM_ATMOSPHERIC_CHEMISTRY_E3SM_IMPLICIT_EULER_HPP__
...
struct AtmosphericChemistryE3SM_ImplicitEuler
{
    ...
    const KineticModelNCAR_ConstData<DeviceType>& kmcd)
  {
    ...
    if (m < 32) {
      ...
      using problem_value_type = Impl::AtmosphericChemistryE3SM_Problem<value_type, device_type>;     
      work_size = problem_value_type::getWorkSpaceSize(kmcd);
    } else if (m < 64) {
    ...
    Impl::TimeIntegratorImplicitEuler<real_type, device_type>;

    ordinal_type w_time_integration(0);
    time_integrator_type::workspace(w_time_integration, w_time_integration);

    return work_size + w_time_integration;

...
===================================================================================== */
#ifndef __TCHEM_IMPL_KFORWARD_HPP__
#define __TCHEM_IMPL_KFORWARD_HPP__
...
struct KForward
{
    ...
    const kinetic_model_type& kmcd)
  {
    ...
    const real_type conv = kmcd.CONV_PPM * p * t_1;
    ...
    Kokkos::parallel_for(
      Tines::RangeFactory<value_type>::TeamVectorRange(member, n_arrhenius_reac), [&](const ordinal_type& i) {
        ...
        kfor(param._reaction_index) = param._A * ats<real_type>::exp(param._C * t_1) *
                                      ( param._B == 0.0 ? one : ats<real_type>::pow (t/param._D, param._B)) *
                                      ( param._E == 0.0 ? one : (one + param._E*p) );

        // FIXME: e3sm code does not include this unit change..., but cam does.
                                      // *
    ...
    Kokkos::parallel_for(
      Tines::RangeFactory<value_type>::TeamVectorRange(member, n_troe_reactions), [&](const ordinal_type& i) {
        ...
        const real_type k0 = k0_A * ( k0_C == 0.0 ? one : ats<real_type>::exp(k0_C * t_1) ) *
                             ( k0_B == 0.0 ? one : ats<real_type>::pow (t/real_type(300.0), k0_B) );
                             // *
                             // conv;

        const real_type k0_kinf = k0 / ( kinf_A * ( kinf_C == 0.0 ? one :ats<real_type>::exp(kinf_C * t_1) )*
                                  ( kinf_B == 0.0 ? one : ats<real_type>::pow (t/real_type(300.0), kinf_B) ) );
        ...
        kfor(idx_reac) =  k0 / ( real_type(1.0) + k0_kinf ) *
                          ats<real_type>::pow( Fc, real_type(1.0) / ( real_type(1.0) +
                          ats<real_type>::pow( ats<real_type>::log10(k0_kinf) / N , real_type(2.0) ) ) );
                           // *
    ...
    Kokkos::parallel_for(
      Tines::RangeFactory<value_type>::TeamVectorRange(member, count_cmaq_h2o2_reac), [&](const ordinal_type& i) {
        ...
        kfor(param._reaction_index) = (param._A1 * ats<real_type>::exp(param._C1 * t_1) *
                                      ( param._B1 == 0.0 ? one : ats<real_type>::pow (t/real_type(300.0), param._B1)) +
                                      param._A2 * ats<real_type>::exp(param._C2 * t_1) *
                                      ( param._B2 == 0.0 ? one : ats<real_type>::pow (t/real_type(300.0), param._B2))*conv) *
                                      ats<real_type>::pow (conv, kmcd.convExponent(param._reaction_index)) ;

...
void form_caco3(Ensemble *ensemble) {
  ensemble->process([=](const Input &input, Output &output) {
    ...
    auto jp_arr = input.get_array("jp");
    ...
    std::string profile_name ="Verification_test_form_caco3";
    using policy_type =
    ...
    TChem::Impl::MOSAIC<real_type, device_type>::form_caco3(
      mmd,
      jp(0),
      electrolyte,
      aer,
    ...
    std::vector<Real> store_out(mmd.naer), electrolyte_out(mmd.nelectrolyte), aer_out(mmd.naer);

...
int main(int argc, char *argv[]) {
  ...
  std::string outputFile("GasChemistry.dat");
  ...
  opts.set_option<std::string>("chemfile", "Chem file name e.g., chem.inp",
                               &chemFile);
  //opts.set_option<std::string>("aerofile", "Aerosol Chem file name e.g., aero.yaml", &aeroFile);
  opts.set_option<std::string>("inputfile", "Chem file name e.g., chem.inp",
                               &inputFile);
  //opts.set_option<std::string>("inputfile_particles", "input file name e.g., chem.inp", &input_file_particles);
 ...
 // read scenario condition from yaml file
    ...
    const ordinal_type per_team_extent = problem_type::getWorkSpaceSize(kmcd)
    + number_of_equations;
    ...
    {
      fprintf(fout_times, "{\n");
      fprintf(fout_times, " \"Gas RHSs\": \n {\n");
      const ordinal_type level = 1;
      const std::string profile_name = "TChem::GasChemistry::RHS_evaluation"; // NOTE: Does this need to be AtmosphericChemistry
      Kokkos::fence();
      timer.reset();
    ...
    {
      const ordinal_type level = 1;
      fprintf(fout_times, " \"Gas Numerical Jacobian\": \n {\n");
      const std::string profile_name = "TChem::GasChemistry::NumericalJacobian_evaluation"; // NOTE: Does this need to be AtmosphericChemistry
      Kokkos::fence();
      timer.reset();
      Kokkos::Profiling::pushRegion(profile_name);
      Kokkos::parallel_for

...
Fork of TChem_AerosolChemistry_RHSs.cpp by Sam Frederick, July 2026
=====================================================================================
*/
#include "TChem.hpp"
...
int main(int argc, char *argv[]) {
  ...
  opts.set_option<bool>("do_rhs_adapter", "Evaluate RHS (Adapter Test)", &do_rhs_adapter);
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    real_type_2d_view_type rhs_adapter("rhs_adapter", nBatch, number_of_equations);
    ...
    if (do_rhs){
      printf("..evaluating RHS (direct via team_invoke)\n");
      fprintf(fout_times, " \"Aerosol RHSs\": \n {\n");
      const ordinal_type level = 1;
      const std::string profile_name = "TChem::AerosolChemistry::RHS_evaluation_via_teaminvoke";
      Kokkos::fence();
      timer.reset();
    ...
    if (do_rhs_adapter){
      // -----------------------------------------------------------------------------------
      // Adapter test for RHS struct 

      printf("..evaluating RHS (via AerosolChemistryRHS->Problem\n");
      fprintf(fout_times, " \"Aerosol RHSs\": \n {\n");
      const ordinal_type level = 1;
      const std::string profile_name = "TChem::AerosolChemistry::RHS_evaluation_via_problem";
      Kokkos::fence();
      timer.reset();
      Kokkos::Profiling::pushRegion(profile_name);
      ...
      Kokkos::parallel_for
      (profile_name,
       policy, // @Claude is this a MD range policy based on keyword args for team size and vector size??
       KOKKOS_LAMBDA(const typename policy_type::member_type& member) {

        const ordinal_type i = member.league_rank();
        const real_type_1d_view_type vals_i  = Kokkos::subview(y2d,         i, Kokkos::ALL()); // y2d instead of state since we just want the ODE variables (no temp/press)
        const real_type_1d_view_type omega_i = Kokkos::subview(rhs_adapter, i, Kokkos::ALL());
        ...
        auto rhs_slice = batched_state.team_slice(i, i + 1, pw);
        rhs_slice(member, vals_i, omega_i);
    ...
    if (do_rhs && do_rhs_adapter){
      auto rhs_host= Kokkos::create_mirror_view(rhs);
      Kokkos::deep_copy(rhs_host, rhs);
      auto rhs_adapter_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), rhs_adapter);
      real_type max_err = 0.0;
      for (ordinal_type i = 0; i < nBatch; i++){
        for (ordinal_type j = 0; j < number_of_equations; j++) {
          // compute the relative error between the team_invoke approach and the adapter approach
          // for rhs_host entries with zero magnitude, set the error to absolute error
          const real_type num = std::abs(rhs_host(i,j) - rhs_adapter_host(i,j));
          const real_type den = std::abs(rhs_host(i,j));
          const real_type err = (den > 0) ? num / den : num;   // ref==0 -> absolute
          max_err = std::max(max_err, err);
        }
      }
      std::printf("Results\n");
      // guard against a vacuous pass: max_err starts at 0, so report how many entries were
      // actually compared -- 0 x 0 would print 0.000e+00 and "PASS" without testing anything.
      std::printf("  entries compared                   : %d x %d\n", nBatch, number_of_equations);
      std::printf("  max ||rhs-rhs_adapter||/||rhs||    : %.3e\n", max_err);
      bool pass = (max_err < 1e-12); // should expect really tight tolerance on the relative error
      std::printf("\n%s\n", pass ? "PASS" : "FAIL");
    }

...
def create_dataFrame_cpu(flist):
  ...
  for file in flist:
    ...
    df.sort_values(by=['nBatch', 'nThread'], axis=1, inplace=True)
    df = df.transpose()
  return df
...
def create_dataFrame_gpu(flist):
  ...
  for file in flist:
    ...
    colName = '_' + nBatch+'t_'+ team_size  +'v_'+ vector_size
  ...
  df = pd.concat(df_list, axis=1)
  df.sort_values(by=['nBatch', 'vector_size','team_size'], axis=1, inplace=True)
  df = df.transpose()
  return df
